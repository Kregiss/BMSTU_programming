## Содержание
### 1 семестр
* [Алгоритмы и структуры данных (C)](./Algorithms%20and%20data%20structures/)
* [Основы информатики (Scheme)](./Basics%20of%20informatic/)
### 2 семестр  
* [Дискретная математика (Go)](./Discrete%20mathematic/)
* [Языки и методы программирования (Java, C++)](./Programming%20languages%20and%20methods/)
### 3 семестр
* [Компьютерные сети (Go)](./Computer%20networks/)
* [Низкоуровневое программирование (Assembler)](./Low-level%20programming/)
### 4 семестр
* [Алгоритмы компьютерной графики (Python)](./Computer%20Graphics%20Algorithms/)
* [Операционые системы (ReactOS, NetBSD)](./Operating%20Systems/)
### 5 семестр
* [Базы данных (SQL Server)](.//Data%20Bases/)
* [Разработка параллельных и распределённых программ (Java, C++)](./RPiRP/)
* [Теория формальных языков (C, Python)](https://github.com/Kregiss/TFL/)
### 6 семестр
* [Генерация оптимального кода (C++)](.//Generating%20optimal%20code/)
* [Конструирование компиляторов (Java, Python)](.//Compilers)
* [Объектно-функциональное программирование (Scala)](.//Object-functional%20programming/)
* [Численные методы (Python)](.//Numerical%20methods/)
